...
function App() {
  ...
  const LogoComponent = ({ business, size = "large" }: { 
    ...
    const sizeClasses = size === "large" ? "h-20 w-20" : size === "medium" ? "h-16 w-16" : "h-12 w-12";
    const iconSizeClasses = size === "large" ? "w-10 h-10" : size === "medium" ? "w-8 h-8" : "w-6 h-6";
    const paddingClasses = size === "large" ? "p-4" : size === "medium" ? "p-3" : "p-2";
